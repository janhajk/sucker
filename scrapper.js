        ...
        http.get(feed.url, function (res) {
                ...
                res.on('end', function () {
                        ...
                        if (!body || res.statusCode !== 200)
                                return callback({message: feed.url + ': No content found!'});

                        parseXML(body, function (err, rss) {
                                if (err)
                                        return callback({message: feed.url + ' is not a xml File!'});

                                feed = parseRSS(rss);
                                if (!feed)
                                        feed = parseAtom(rss);
                                if (!feed)
                                        return callback({message: 'No scrapper found for ' + feed.url + '!'});
                                callback(err, feed);
                        });

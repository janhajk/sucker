  ...
  $(document).ready(function() {      
    ...
    var loadRssTV = function(type) {
      ...
      $.getJSON('/site/rss/'+type, function(json) {
        ...
        for (var i in json) {
          rss.push(json[i]);
          html += '<tr><td><a href="javascript:" id="rss_'+type+'_items_'+i+'" class="rss_'+type+'_items">' + json[i].title + '</a></td><td></td></tr>';
        }
    ...
    var loadFiles = function() {
        $.getJSON('/file', function(files) {
            ...
            files.sort(function(a,b){a=a.filename.toLowerCase();b=b.filename.toLowerCase();return a>b?1:a<b?-1:0;});
            for (var i in files) {
                $('#rss_files tbody').append('<tr><td><a href="'+files[i].link+'">'+files[i].filename+'</a></td>'
   ...
   var setStatus = function(msg, done) {
     $('#status').show(200);  
     $('#status').html(msg);
     if (done === 'fadeout') {
         setTimeout(function(){$('#status').hide(500)}, 5000);
     }
   };
   ...
   var ripSite = function(url) {
    ...
    var parseIDs = function(ids) {
       ...
       var addFile = function(info) {
         ...
         curfile++;
         setStatus((filescount-curfile) + ' files left to check...');
         makeTableChecked(files);
       };
       setStatus('start checking ' + filescount + ' files...');
       $.each(ids, function(key, value){
         defs.push($.getJSON('/ul/'+value+'/check', function(info){addFile(info);}));
       });
       $.when.apply(null, defs).done(function() {
         setStatus(filescount + ' files checked!', 'fadeout');
         makeTableChecked(files);
       });
    };
    ...
    var makeTableChecked = function(fileList) {
      ...
      fileList.sort(function(a,b){return a.extension<b.extension?1:a.extension>b.extension?-1:0;}); // sort files by extension
